...
def get_optimizer(
    ...
    if args.main_optim == 'SGD':
        ...
        # Training parameters
        return torch.optim.AdamW(parameters, lr=args.learning_rate, eps=args.adam_eps)
...
class OptimScheduler(object):
    ...
    def init_optimizer(self, net_params):
        optimizer = None
        if self.configer.get('optim', 'optimizer') == 'sgd':
        ...
        elif self.configer.get('optim', 'optimizer') == 'adam':
        ...
        elif self.configer.get('optim', 'optimizer') == 'adamw':
            optimizer = AdamW(net_params,
                              lr=self.configer.get('lr', 'base_lr'),
                              eps=self.configer.get('optim', 'adamw')['eps'])

        else:
            Log.error('Optimizer {} is not valid.'.format(self.configer.get('optim', 'optimizer')))
            exit(1)
    ...
    def update_optimizer(self, net, optim_method, lr_policy):
        self.configer.update(('optim', 'optimizer'), optim_method)
