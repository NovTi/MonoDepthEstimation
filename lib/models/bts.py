...
class bts(nn.Module):
    def __init__(self, feat_out_channels, num_features=512):
        ...
        self.daspp_conv = torch.nn.Sequential(nn.Conv2d(num_features + num_features // 2 + num_features // 4, num_features // 4, 3, 1, 1, bias=False),
                                              nn.ELU())
        self.reduc8x8   = reduction_1x1(num_features // 4, num_features // 4, get('data', 'max_depth'))
        ...
        self.conv3      = torch.nn.Sequential(nn.Conv2d(num_features // 4 + feat_out_channels[1] + 1, num_features // 4, 3, 1, 1, bias=False),
                                              nn.ELU())
        self.reduc4x4   = reduction_1x1(num_features // 4, num_features // 8, get('data', 'max_depth'))
        ...
        self.reduc2x2   = reduction_1x1(num_features // 8, num_features // 16, get('data', 'max_depth'))
        self.lpg2x2     = local_planar_guidance(2)
        
        self.upconv1    = upconv(num_features // 8, num_features // 16)
        self.reduc1x1   = reduction_1x1(num_features // 16, num_features // 32, get('data', 'max_depth'), is_final=True)
    ...
    def forward(self, features, focal):
        ...
        iconv5 = self.conv5(concat5)   # [B, 512, 26, 34]
        
        upconv4 = self.upconv4(iconv5) # H/8   # [B, 256, 52, 68]
        upconv4 = self.bn4(upconv4)
        concat4 = torch.cat([upconv4, skip2], dim=1)
        iconv4 = self.conv4(concat4)
        iconv4 = self.bn4_2(iconv4)   # [B, 256, 52, 68]
        
        daspp_3 = self.daspp_3(iconv4)   # [B, 128, 52, 68]
        concat4_2 = torch.cat([concat4, daspp_3], dim=1)  # [B, 576, 52, 68]
        daspp_6 = self.daspp_6(concat4_2)                 # [B, 128, 52, 68]
        concat4_3 = torch.cat([concat4_2, daspp_6], dim=1) # [B, 704, 52, 68]
        daspp_12 = self.daspp_12(concat4_3)            # [B, 128, 52, 68]
        concat4_4 = torch.cat([concat4_3, daspp_12], dim=1)  # [B, 832, 52, 68]
        daspp_18 = self.daspp_18(concat4_4)                # [B, 128, 52, 68]
        concat4_5 = torch.cat([concat4_4, daspp_18], dim=1)   # [B, 960, 52, 68]
        daspp_24 = self.daspp_24(concat4_5)                 # [B, 128, 52, 68]
        concat4_daspp = torch.cat([iconv4, daspp_3, daspp_6, daspp_12, daspp_18, daspp_24], dim=1)  # [B, 896, 52, 68]
        daspp_feat = self.daspp_conv(concat4_daspp)    # [B, 128, 52, 68]
        
        reduc8x8 = self.reduc8x8(daspp_feat)   # [B, 4, 52, 68]
        plane_normal_8x8 = reduc8x8[:, :3, :, :]   # [B, 3, 52, 68]
        plane_normal_8x8 = torch_nn_func.normalize(plane_normal_8x8, 2, 1)
        plane_dist_8x8 = reduc8x8[:, 3, :, :]    # [B, 1, 52, 68]
        plane_eq_8x8 = torch.cat([plane_normal_8x8, plane_dist_8x8.unsqueeze(1)], 1)  # [B, 4, 52, 68]
        depth_8x8 = self.lpg8x8(plane_eq_8x8, focal)    # [B, 416, 544]   expand 8 times
        depth_8x8_scaled = depth_8x8.unsqueeze(1) / get('data', 'max_depth')   # [B, 1, 416, 544]
        depth_8x8_scaled_ds = torch_nn_func.interpolate(depth_8x8_scaled, scale_factor=0.25, mode='nearest')
        
        upconv3 = self.upconv3(daspp_feat) # H/4   # [B, 128, 104, 136]
        upconv3 = self.bn3(upconv3)
        concat3 = torch.cat([upconv3, skip1, depth_8x8_scaled_ds], dim=1)   # [B, 225, 104, 136]
        iconv3 = self.conv3(concat3)   # [B, 128, 104, 136]
        
        reduc4x4 = self.reduc4x4(iconv3)   # [B, 4, 104, 136]
        ...
        depth_4x4_scaled = depth_4x4.unsqueeze(1) / get('data', 'max_depth')
        ...
        depth_2x2_scaled = depth_2x2.unsqueeze(1) / get('data', 'max_depth')
        ...
        final_depth = get('data', 'max_depth') * self.get_depth(iconv1)
        if get('data', 'dataset') == 'kitti':
...
class encoder(nn.Module):
    def __init__(self):
        ...
        if get('network', 'encoder') == 'densenet121_bts':
        ...
        elif get('network', 'encoder') == 'densenet161_bts':
            self.base_model = models.densenet161(pretrained=True).features
            self.feat_names = ['relu0', 'pool0', 'transition1', 'transition2', 'norm5']
            self.feat_out_channels = [96, 96, 192, 384, 2208]
        elif get('network', 'encoder') == 'resnet50_bts':
            self.base_model = models.resnet50(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif get('network', 'encoder') == 'resnet101_bts':
            self.base_model = models.resnet101(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif get('network', 'encoder') == 'resnext50_bts':
            self.base_model = models.resnext50_32x4d(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif get('network', 'encoder') == 'resnext101_bts':
            self.base_model = models.resnext101_32x8d(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif get('network', 'encoder') == 'mobilenetv2_bts':
...
class BtsModel(nn.Module):
    ...
    def forward(self, x, focal):
        # x: [B, 3, 416, 544]
        # [0]: [B, 96, 208, 272]   [1]: [B, 96, 104, 136]
        # [2]: [B, 192, 52, 68]    [3]: [B, 384, 26, 34]
        # [4]: [B, 2208, 13, 17]
